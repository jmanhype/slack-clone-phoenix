import { NotionDatabase, NotionPage, ConversionResult, PageMetadata, ImportError } from '../types';
import { ContentConverter } from './ContentConverter';
import { createLogger } from '../utils/logger';

const logger = createLogger('DatabaseConverter');
...
export class DatabaseConverter {
  ...
  constructor(_config?: any) {
    ...
    this.contentConverter = new ContentConverter(_config);
  ...
  async convertDatabase(
    ...
    const pageFiles: ConversionResult[] = [];
    ...
    try {
      ...
      for (const page of pages) {
        try {
          ...
          const pageResult = await this.contentConverter.convertBlocks(
            page.children || [],
            pageMetadata
          );
      ...
      const indexFile: ConversionResult = {
   ...
   */
  private generateDatabaseIndex(database: NotionDatabase, pages: NotionPage[]): string {
    const lines: string[] = [];
    ...
    if (Object.keys(database.properties).length > 0) {
      ...
      for (const [propName, propConfig] of Object.entries(database.properties)) {
        const _config = propConfig as any;
        ...
        if (_config.type === 'select' && _config.select?.options) {
          const options = _config.select.options.map((opt: any) => opt.name).join(', ');
          lines.push(`  - Options: ${options}`);
        } else if (_config.type === 'multi_select' && _config.multi_select?.options) {
          const options = _config.multi_select.options.map((opt: any) => opt.name).join(', ');
          lines.push(`  - Options: ${options}`);
   ...
   */
  private createPageMetadata(page: NotionPage, database: NotionDatabase): PageMetadata {
    const tags: string[] = [];
    ...
    for (const [, propValue] of Object.entries(page.properties || {})) {
      ...
      if (value.type === 'multi_select' && value.multi_select) {
        tags.push(...value.multi_select.map((option: any) => option.name));
   ...
   */
  private createDatabaseMetadata(database: NotionDatabase): PageMetadata {
    return {
      ...
      switch (value.type) {
        ...
        case 'rich_text':
          if (value.rich_text?.[0]?.plain_text) {
            processed[propName] = value.rich_text.map((rt: any) => rt.plain_text).join('');
          }
          break;
        ...
        case 'multi_select':
          if (value.multi_select?.length > 0) {
            processed[propName] = value.multi_select.map((option: any) => option.name);
          }
          break;
        ...
        case 'relation':
          if (value.relation?.length > 0) {
            processed[propName] = value.relation.map((rel: any) => rel.id);
          }
          break;
        ...
        case 'people':
          if (value.people?.length > 0) {
            processed[propName] = value.people.map((person: any) => person.name || person.id);
          }
          break;

        case 'files':
          if (value.files?.length > 0) {
            processed[propName] = value.files.map((file: any) => 
              file.name || file.external?.url || file.file?.url
            );
          }
          break;
   ...
   */
  private getRelevantProperties(databaseProperties: any): string[] {
   ...
   */
  private createTableRow(page: NotionPage, propertyNames: string[]): string[] {
    const row: string[] = [];
   ...
   */
  private formatPropertyForTable(propValue: any): string {
    if (!propValue) return '';
    ...
    switch (propValue.type) {
      ...
      case 'multi_select':
        return propValue.multi_select?.map((opt: any) => opt.name).join(', ') || '';
      ...
      case 'formula':
        if (propValue.formula?.string) return propValue.formula.string;
        if (propValue.formula?.number !== null) return propValue.formula.number.toString();
        if (propValue.formula?.boolean !== null) return propValue.formula.boolean ? '✓' : '';
        return '';
   ...
   */
  private generatePageFilename(page: NotionPage): string {
